...
class BaseController:
    ...
    # --- 共通のUIトリガーメソッド ---
    def add(self): self.layout.open_add_dialog()
    def add_random(self): self.layout.open_random_dialog()
    def edit(self):
    ...
    def act(self): self.layout.open_act_dialog()
    ...
    def execute_act(self, choice):
        if choice not in self.manager.ALLOWED_ACTIONS:
    ...
    def save(self):
        if self.backend:
            ...
            self.layout.log("保存先のバックエンドが設定されていません")

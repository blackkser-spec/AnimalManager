...
def sample_animal(client):
    ...
    response = client.post("/animals", json={"animal_type": "cat", "name": "TestCat"})
    return response.json()

class TestAddAnimal:
    def test_success(self, client):
        payload = {"animal_type": "cat", "name": "AddCat"}
    ...
    def test_error_save_failed(self, client):
        """追加時の保存失敗(500)のテスト"""
        manager.repository.save.side_effect = SaveError("save_error")
        payload = {"animal_type": "cat", "name": "FailAnimal"}
        response = client.post("/animals", json=payload)
        assert response.status_code == 500

    def test_error_empty_name(self, client):
        payload = {"animal_type": "cat", "name": ""}
        response = client.post("/animals", json=payload)
        assert response.status_code == 400
        assert response.json()["key"] == "name_empty"
...
class TestAddRandomAnimal:
    ...
    def test_error_negative_value(self, client):
        ...
        assert response.status_code == 400
        assert response.json()["key"] == "require_positive_int"
...
class TestRemoveAnimal:
    ...
    def test_error_invalid_id(self, client, animal_id, expected_status):
        ...
        if expected_status == 404:
            assert response.json()["key"] == "id_not_found"


class TestEditAnimal:
    def test_name_success(self, client, sample_animal):
        response = client.patch(f"/animals/{sample_animal['id']}", json={"animal_type": "dog", "name": "EditedDog"})
        assert response.status_code == 200
...
class TestActAnimal:
    def test_success_sound(self, client, sample_animal):
        response = client.get("/animals/act/sound")
        assert response.status_code == 200
        results = response.json()
        assert isinstance(results, list)
        assert len(results) == 1
        assert results[0]["name"] == sample_animal["name"]
...
class TestGetAnimal:
    def test_get_detail_success(self, client, sample_animal):
        ...
        assert response.json()["animal_type"] == "cat"
...
class TestSearchAnimal:
    ...
    def test_search_with_sort(self, client):
        client.post("/animals", json={"animal_type": "cat", "name": "B_Cat"})
        client.post("/animals", json={"animal_type": "cat", "name": "A_Cat"})
        # 名前順でソート
        response = client.get("/animals", params={"sort_by": "name"})
